...
**Key dependencies:** LangChain, LangGraph, langchain-community arXiv **search** tools (`ArxivRetriever` / `ArxivQueryRun`), `langchain_text_splitters`, BeautifulSoup/lxml/markdownify (HTML ingest), Voyage embeddings `voyage-4-large` (1024-d), Voyage `rerank-3` (retrieve scorer), OpenAI (`gpt-5.1`, `gpt-5-mini`) via `langchain-openai` **chat**, Chainlit, psycopg/pgvector
...
- Embeddings: Voyage (`voyage-4-large`, 1024-d). LLM vendor: OpenAI (`gpt-5.1` / `gpt-5-mini`).
