...
**Embeddings amendment (Execute 2026-09-08):** `.specs/features/voyage-4-large-embeddings/` supersedes STORE-01 **width** `vector(1536)` (columns / identity unchanged: `kind`, `unit_id`, dual text, JSONB keys, atomic identity). `ensure_schema` missing-`kind` DROP `chunks` / DELETE `papers` at boot is superseded; corpus reset is `scripts/wipe_paper_chunks.py --yes`. Live Independent Tests of this amendment are **not** complete.
